...
| Login mit OTP-Code statt nur Link | Code gebaut, 2 Dashboard-Schritte offen | UI fertig (`app/login/page.tsx`, `supabase.auth.verifyOtp`). Nutzer muss noch: 1) im Supabase-Dashboard das Magic-Link-E-Mail-Template um `{{ .Token }}` ergänzen (sonst steht kein Code in der Mail), 2) "Anonymous Sign-Ins" unter Authentication → Providers aktivieren (Voraussetzung für den Gast-Zugang unten). |
